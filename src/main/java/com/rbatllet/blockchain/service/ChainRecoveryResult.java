...
public class ChainRecoveryResult {
    ...
    public enum RecoveryStatus {
        ...
        PARTIAL_SUCCESS("⚠️ Partial Recovery", "Some issues remain after recovery"),
    ...
    public static class RecoveryAction {
        ...
        public RecoveryAction(String actionType, Long blockNumber, String description, 
    ...
    public static class RecoveryStatistics {
        ...
        // Getters
        public int getTotalBlocksAnalyzed() { return totalBlocksAnalyzed; }
        public int getCorruptedBlocksFound() { return corruptedBlocksFound; }
        public int getBlocksRepaired() { return blocksRepaired; }
        public int getBlocksRolledBack() { return blocksRolledBack; }
        public int getOrphanBlocksRemoved() { return orphanBlocksRemoved; }
        public long getDataBytesRecovered() { return dataBytesRecovered; }
    ...
    public ChainRecoveryResult(String recoveryId, RecoveryStatus status, String message) {
    ...
    public ChainRecoveryResult(String recoveryId, RecoveryStatus status, String message,
                             ...
                             List<RecoveryAction> actions, RecoveryStatistics statistics) {
        this.recoveryId = recoveryId;
        this.status = status;
        this.message = message;
    ...
    public Map<String, List<RecoveryAction>> groupActionsByType() {
        ...
        for (RecoveryAction action : actions) {
            grouped.computeIfAbsent(action.getActionType(), k -> new ArrayList<>()).add(action);
        }
        return grouped;
    }
    ...
    public String getFormattedSummary() {
        ...
        sb.append(String.format("  - Blocks Analyzed: %d\n", statistics.getTotalBlocksAnalyzed()));
        sb.append(String.format("  - Corrupted Found: %d\n", statistics.getCorruptedBlocksFound()));
        sb.append(String.format("  - Blocks Repaired: %d\n", statistics.getBlocksRepaired()));
        sb.append(String.format("  - Blocks Rolled Back: %d\n", statistics.getBlocksRolledBack()));
        sb.append(String.format("  - Orphans Removed: %d\n", statistics.getOrphanBlocksRemoved()));
        sb.append(String.format("  - Data Recovered: %.2f MB\n", statistics.getDataBytesRecovered() / (1024.0 * 1024.0)));
        sb.append(String.format("  - Success Rate: %.2f%%\n\n", statistics.getSuccessRate()));
        
        // Actions summary
        if (!actions.isEmpty()) {
            sb.append("🔄 Recovery Actions:\n");
            Map<String, List<RecoveryAction>> groupedActions = groupActionsByType();
            for (Map.Entry<String, List<RecoveryAction>> entry : groupedActions.entrySet()) {
                long successful = entry.getValue().stream().mapToLong(action -> action.isSuccess() ? 1 : 0).sum();
                sb.append(String.format("  - %s: %d/%d successful\n", 
                    entry.getKey(), successful, entry.getValue().size()));
            }
            sb.append("\n");
        }
